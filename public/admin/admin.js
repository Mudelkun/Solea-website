...
function displayProducts() {
  ...
  if (products.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="7" class="loading">Aucun produit trouvé</td></tr>';
    return;
  }
...
function openProductModal(product = null) {
  ...
  if (product) {
    ...
    document.getElementById("product-visible").value =
    ...
    if (product.hairType) {
      product.hairType.forEach((type) => {
        const cb = form.querySelector(
          `input[name="hairType"][value="${type}"]`,
        );
    ...
    if (product.variants && product.variants.length > 0) {
      product.variants.forEach((variant) =>
        addVariant(variant.name, variant.price),
      );
    }
  } else {
    // Add mode
  ...
  form.addEventListener("submit", async function (e) {
    ...
    document.querySelectorAll(".variant-row").forEach((row) => {
      ...
      const price = parseFloat(
        row.querySelector('input[name="variant-price"]').value,
      );
      if (name && !isNaN(price)) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // For new products, the ID is in responseData.product.id; for updates, use the original productId
        const savedProductId =
          responseData.product?.id || responseData.id || productId;
        ...
        if (imageInput && imageInput.files && imageInput.files.length > 0) {
          ...
          if (!imageResponse.ok) {
            ...
            alert(
              "Produit sauvegardé mais erreur lors de l'upload des images: " +
                (errorData.error || ""),
            );
          }
        }
...
async function loadSettings() {
  try {
    ...
    document.getElementById("business-name").value =
      settings.business?.name || "Solea";
    document.getElementById("free-shipping").value =
      settings.business?.freeShippingThreshold || 50;
    document.getElementById("shipping-cost").value =
      settings.business?.shippingCost || 5.99;
